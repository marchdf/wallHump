...
def parse_surface_probe(fname, yname):
    ...
    tau_wall_all = msh.variables["vals_nod_var%d" % (tau_idx + 1)][:][1:, wall_nids]
...
if __name__ == "__main__":
    ...
    for k, fdir in enumerate(fdirs):
        ...
        yname = os.path.join(os.path.abspath(fdir), "wallHump.yaml")
        u0, rho0, mu = parse_ic(yname)
    ...
    plt.figure(0)
    p = plt.plot(
        [nmin, nmax], [separation_exp, separation_exp], color=cmap[-1], label="Exp"
    )
    plt.figure(1)
    p = plt.plot(
        [nmin, nmax],
